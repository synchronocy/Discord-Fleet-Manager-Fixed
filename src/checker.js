...
function check(config) {
    ...
    new Bot(token, proxy).req("GET", "https://discordapp.com/api/v6/users/@me").then(body => {
        ...
        if (parsed.code == "0") {
            console.log("Token " + token + chalk.red(" failed (maybe the token got phone secured)"));
            set(token, false);
        } else {
            console.log("Token " + token + chalk.green(" passed "));
            set(token, true);
            t++;
        }

        console.log("Proxy " + proxy + chalk.green(" passed "));
        set(proxy, true);
        ...
        console.log("Proxy " + proxy + chalk.red(" failed ✗") + " (proxy timed out > 2500ms)");
        set(proxy, false);
