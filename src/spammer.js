...
function start(body) {
    ...
    function join() {
        if (!active) return console.log(chalk.red("Stopped ✗"));
        ...
        bot.req("POST", "https://discordapp.com/api/v6/invite/" + attack.invite).then(body => {
            ...
            if (!parsed.guild.id) {
                console.log(chalk.red("A bot died ✗") + " (Failed to join)");
                kill();
            } else {
                ...
                function message() {
                    ...
                    if (bot.fails >= 3) {
                        console.log(chalk.red("A bot died ✗"));
                        kill();

                        // If no more bots are alive stop
                        if (aliveBots.length == 0) {
                            console.log(chalk.red("Stopped ✗") + " (No more bots)");
                            active = false;
                        }
                    }
